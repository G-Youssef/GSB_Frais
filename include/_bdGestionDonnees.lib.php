...
/**
 * Modifie le libelle d'une ligne hors forfait pour le refuser, et le d�tache de
 * la fiche courante en affectant une date nulle � "mois".
 * @param string $uneDate       date du hors forfait (MMAAAA) 
 * @param string $unIdVisiteur  id visiteur
 * @param string $unLibelle     le libelle du hors forfait
 * @return void  
 */
 ...
 * @return void 
 */
function modifierEtatFicheFrais($idCnx, $unMois, $unIdVisiteur, $unEtat, $unMontant, $unNbJustificatif) {
    $requete = "update fichefrais set idEtat = '".$unEtat. 
               "', dateModif = now(), montantValide = '".$unMontant."', nbJustificatifs = '".$unNbJustificatif."' 
               where idVisiteur ='" .$unIdVisiteur . "' and mois = '". $unMois . "'";
    mysql_query($requete);
}             

/**
 * Renvoie la cat�gorie du visiteur, s'il est comptable ou visiteur m�dical
 * @param type $idConnexion identifiant de connexion
 * @param type $idUser identifiant de l'utilisateur
 * @return type
 */
function obtenirCategorieVisiteur($idConnexion, $idUser) {
    $req = "select statut from visiteur where id='".$idUser."'";
    $resultat = mysql_query($req, $idConnexion);
    $ligne = false;
    if ( $resultat ) {      $ligne = mysql_fetch_assoc($resultat);
...
/**
 * Met � jour dans la BD la ligne hors forfait de libell� $unLibelleHF du montant $unMontantHF 
 * ayant eu lieu � la date $uneDateHF pour la fiche de frais du mois
 * @param type $idLigneHF identifiant de la ligne hors forfait
 * @param type $uneDateHF date de la ligne hors forfait
 * @param type $unLibelleHF libelle de la ligne hors forfait
 * @param type $unMontantHF montant de la ligne hors forfait
 */
function modifierLigneHF($unIdLigneHF, $uneDateHF, $unLibelleHF,  $unMontantHF) {
    $requete = "update lignefraishorsforfait set date='".$uneDateHF."', libelle='".$unLibelleHF."',  montant=".$unMontantHF." where id =" . $unIdLigneHF;
    mysql_query($requete);
}

/**
 * Met � jour dans la BD l'Etat de la fiche forfait  pour un visiteur et mois donn�e(de "VA" � "MP" et de "MP" � "RB")
 * @param type $idVisiteur
 * @param type $moisFicheFrais
 */
function miseEnPaiementEtRemboursement($idVisiteur,$moisFicheFrais){
    $update = date("Y-m-d");
$req1= "UPDATE fichefrais set idEtat = 'MP', dateModif = '$update' WHERE idVisiteur = '$idVisiteur' AND mois = '$moisFicheFrais'";
    mysql_query($req1); 
    $req2 = "UPDATE fichefrais set idEtat = 'RB', dateModif = '$update' WHERE idVisiteur = '$idVisiteur' AND mois = '$moisFicheFrais'";
    mysql_query($req2);
}
